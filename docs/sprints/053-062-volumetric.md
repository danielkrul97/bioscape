...
## Sprinty 64+ — open-ended

- **Sprint 64+:** Pokud user persistuje GPU full-loop ambition: collision +
  predate wire (atomic deltas, 2-pass apply). Sprint 63 data ukazují že
  net benefit je malý ale infra je hotová.
- **Sprint 64+:** Bigger world expansion (z=30, z=50). CPU SmellField stencil
  scaling: 64×64×16 = 65k → 64×64×64 = 256k voxels = 4× compute. Paralelní
  z-rovinová decomposition zachová excellent CPU performance.
- **Sprint 64+:** progressive cells/world growth — bump default
  `MAX_POPULATION` (1000 → 2000-5000) + measure CPU paralelní cesta.
- **Sprint 64+:** thermal stratification (temperature field z-gradient).
- **Sprint 64+:** light field z-attenuation (photic vs aphotic zones).
