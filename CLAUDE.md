...
- **Split kódu:**
  ...
  - `src/bin/headless/` — bezokenní harness pro batch experimenty (CSV log per generaci, deterministický seed). Tick loop = `world.tick()` v tight CPU loop. Konzumuje stejný `bioscape::sim::World` jako renderer — semantická parita je structural guarantee.
